import triangleVertex from "./shaders/cubesDynamicOffset.vert.wgsl?raw";
import triangleFrag from "./shaders/cubesDynamicOffset.frag.wgsl?raw";
...
function draw(device: GPUDevice, context: GPUCanvasContext, pipelineObj: any) {
	...
	renderPass.setBindGroup(0, pipelineObj.uniformGroup,[0]);
	renderPass.draw(triangle.vertexCount); //定点个数 vertexShader会被并行执行三次
	renderPass.setBindGroup(0, pipelineObj.uniformGroup,[256]);
	renderPass.draw(triangle.vertexCount); //定点个数 vertexShader会被并行执行三次
