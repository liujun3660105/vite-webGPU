...
async function run() {
	const canvas1 = document.querySelector('canvas#webgpu') as HTMLCanvasElement;
	const canvas2 = document.querySelector('canvas#canvas') as HTMLCanvasElement;
	console.log('canvas',canvas1,canvas2);
	if(!canvas1 || !canvas2) throw new Error('No Canvas');
	const { device, format, context, size } = await initWebGPU1(canvas1);
	...
	const sampler = device.createSampler({
		addressModeU:'repeat',// U方向和V方向不同的address策略
		addressModeV:'repeat',
		magFilter:'linear',
		minFilter:'linear',
	});
	...
	draw(device,pipeline,context,vertexObj, colorObj,depthTexture,textureGroup);
	...
	function animation(){
		...
		const mvpMatrix = getMvpMatrix(size,position,rotation,scale);
		device.queue.copyExternalImageToTexture({source:canvas2},{texture},textureSize);
		device.queue.writeBuffer(mvpMatrixBuffer,0,mvpMatrix as Float32Array);
		draw(device,pipeline,context,vertexObj, colorObj,depthTexture,textureGroup);
...
window.addEventListener('resize',()=>{
	size.width = canvas1.width = canvas1.clientWidth * window.devicePixelRatio;
	size.height = canvas1.height = canvas1.clientHeight * window.devicePixelRatio;
	console.log('size',size);
	depthTexture.destroy();
	const newDepthTexture = device.createTexture({
		size,format:'depth24plus',usage:GPUTextureUsage.RENDER_ATTACHMENT
	});
	draw(device,pipeline, context,vertexObj, colorObj,newDepthTexture,textureGroup);
});
{
	const ctx = canvas2.getContext('2d')
  console.log("🚀 ~ file: canvasTexture.ts ~ line 300 ~ run ~ ctx", ctx)
	if(!ctx)
